...
This checklist qualifies an InnAware Support preview before wider field use. Run it on clean or representative Windows 10/11 customer and technician systems over a real Internet path.
...
```bash
git rev-parse HEAD
systemctl is-active innaware-support.service
curl -fsS http://127.0.0.1:8787/api/health
curl -fsS https://remote.innawareucp.com/api/health
sha256sum /opt/innaware-support/downloads/InnAwareSupport.exe
sha256sum /opt/innaware-support/downloads/InnAware-Support-Technician-Portable.zip
sha256sum /opt/innaware-support/downloads/InnAware-Support-Technician-Setup.exe
```
...
- FileVersion is **0.9.0.0**;
- the installer reports **0.9.0-preview**;
...
Verify its Windows FileVersion is **0.9.0.0**.
