...
internal static class InputInjector
{
    ...
    public static void Apply(JsonElement input)
    {
        if (!input.TryGetProperty("kind", out var kindElement)) return;
        var kind = kindElement.GetString();
        switch (kind)
        {
            case "mouse_move": MouseMove(input); break;
            case "mouse_button": MouseButton(input); break;
            case "mouse_wheel": MouseWheel(input); break;
            case "key": Key(input); break;
        }
    }

    private static void MouseMove(JsonElement input)
    {
    ...
    private static void MouseButton(JsonElement input)
    {
        MouseMove(input);
    ...
    private static void Key(JsonElement input)
    {
        ...
        var down = input.TryGetProperty("down", out var de) && de.GetBoolean();
        var item = new INPUT { type = INPUT_KEYBOARD, U = new INPUTUNION { ki = new KEYBDINPUT { wVk = vk, dwFlags = down ? 0u : KEYEVENTF_KEYUP } } };
    ...
    private static Dictionary<string, ushort> BuildKeys()
    {
        ...
        for (var i = 0; i <= 9; i++) d[$"Digit{i}"] = (ushort)('0' + i);
        for (var i = 1; i <= 12; i++) d[$"F{i}"] = (ushort)(0x6F + i);
        return d;
    }
}
