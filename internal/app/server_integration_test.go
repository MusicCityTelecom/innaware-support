...
func TestIntegrationOperationsHTTPMySQL(t *testing.T) {
    dsn := os.Getenv("INNAWARE_TEST_MYSQL_DSN")
    if dsn == "" {
        t.Skip("INNAWARE_TEST_MYSQL_DSN is not set")
    }
    if !strings.Contains(dsn, "/innaware_support_ci?") {
        t.Fatal("integration test requires the dedicated innaware_support_ci database")
    }
    ...
    cfg := Config{
        PublicBaseURL:     "https://remote.innawareucp.com",
    ...
    doJSON := func(method, path string, body any, cookie *http.Cookie) *httptest.ResponseRecorder {
        ...
        req := httptest.NewRequest(method, "https://remote.innawareucp.com"+path, bytes.NewReader(raw))
        if body != nil {
            req.Header.Set("Content-Type", "application/json")
        }
        if method != http.MethodGet && method != http.MethodHead {
            req.Header.Set("Origin", "https://remote.innawareucp.com")
        }
        if cookie != nil {
