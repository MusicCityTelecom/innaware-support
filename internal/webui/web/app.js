...
async function ensureH264Decoder(){
  ...
  const decoder=new VideoDecoder({
    output:(frame)=>{
      const started=state.h264DecodeStarts.get(Number(frame.timestamp));
      if(started!==undefined){
        state.renderDecodeMs+=performance.now()-started;
        state.h264DecodeStarts.delete(Number(frame.timestamp));
      }
...
function fallbackToJpeg(reason){
  ...
  state.videoTransport='jpeg';
  $('videoTransportSelect').value='jpeg';
  if($('viewerCodecCapability'))
    $('viewerCodecCapability').textContent=
      `${$('viewerCodecCapability').textContent.split(' · fallback:')[0]} · fallback: ${reason}`;
  sendCaptureSettings();
}
